...
const CreatePostForm = ({closeModal}) => {
    ...
    const publish = async () => {
        setDisable(true);
        if(originalImg || content) {
            let formData = new FormData();
            if (originalImg) formData.append('photo', originalImg);
            formData.set('content', content);
            dispatch(newPost(formData, {headers: {'content-type': 'multipart/form-data'}})).then(() => {
                closeModal();
                alert.success('Publicación exítosa');
            }).catch((e) => {
                alert.error('Alguno de los datos que ingresastes no son válidos, intentalo nuevamente');
            });
        } else {
            alert.error('Para crear una publicación debes ingresar una imagen o un texto...');
        }
        setDisable(false)
    }
            ...
            <div className="mt-2 p-4 text-sm flex flex-col">
                ...
                <div className="w-full flex justify-end">
                    <label
                        ...
                        <input id="file-upload" required={!content} ref={ref => imgValue = ref}
